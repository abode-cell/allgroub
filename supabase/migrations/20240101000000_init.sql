...
BEGIN
    ...
    IF user_role_text = 'مدير المكتب' THEN
        ...
        -- Get default trial days from app_config
        SELECT (value->>'value')::INT INTO trial_days FROM public.app_config WHERE key = 'defaultTrialPeriodDays';

        -- Insert the manager into the public.users table with the new office_id
    ...
    -- If the user is a subordinate (employee, assistant, investor)
    ELSE
