...
CREATE POLICY "Allow all authenticated users to read investors" ON public.investors FOR SELECT USING (true);
...
CREATE POLICY "Allow permitted assistants/employees to manage investors" ON public.investors FOR ALL USING ((SELECT permissions->>'manageInvestors' FROM public.users WHERE id = auth.uid())::boolean);

-- Policies for 'borrowers' table
CREATE POLICY "Allow all authenticated users to read borrowers" ON public.borrowers FOR SELECT USING (true);
...
CREATE POLICY "Allow permitted assistants/employees to manage borrowers" ON public.borrowers FOR ALL USING ((SELECT permissions->>'manageBorrowers' FROM public.users WHERE id = auth.uid())::boolean);
CREATE POLICY "Allow employees with loan edit permissions to update" ON public.borrowers FOR UPDATE USING ( (SELECT "allowEmployeeLoanEdits" FROM public.users WHERE id = (SELECT "managedBy" FROM public.users WHERE id = auth.uid())) = true );
...
CREATE POLICY "Allow authenticated to read transactions" ON "public"."transactions" FOR SELECT TO authenticated USING (true);
