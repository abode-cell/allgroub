...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [supportTickets, setSupportTickets] = useState<SupportTicket[]>(initialData.supportTickets);
  const [notifications, setNotifications] = useState<Notification[]>(initialData.notifications);
  const [salaryRepaymentPercentage, setSalaryRepaymentPercentage] = useState<number>(initialData.salaryRepaymentPercentage);
  const [baseInterestRate, setBaseInterestRate] = useState<number>(initialData.baseInterestRate);
  const [investorSharePercentage, setInvestorSharePercentage] = useState<number>(initialData.investorSharePercentage);
  const [graceTotalProfitPercentage, setGraceTotalProfitPercentage] = useState<number>(initialData.graceTotalProfitPercentage);
  const [graceInvestorSharePercentage, setGraceInvestorSharePercentage] = useState<number>(initialData.graceInvestorSharePercentage);
  const [supportEmail, setSupportEmail] = useState<string>(initialData.supportEmail);
  const [supportPhone, setSupportPhone] = useState<string>(initialData.supportPhone);
      ...
      try {
        ...
        if (item) {
          ...
          if (parsed.users && parsed.borrowers && parsed.investors) {
            ...
            setSupportTickets(parsed.supportTickets || initialData.supportTickets);
            setNotifications(parsed.notifications || initialData.notifications);
            setSalaryRepaymentPercentage(parsed.salaryRepaymentPercentage || initialData.salaryRepaymentPercentage);
            setBaseInterestRate(parsed.baseInterestRate || initialData.baseInterestRate);
            setInvestorSharePercentage(parsed.investorSharePercentage || initialData.investorSharePercentage);
            setGraceTotalProfitPercentage(parsed.graceTotalProfitPercentage || initialData.graceTotalProfitPercentage);
            setGraceInvestorSharePercentage(parsed.graceInvestorSharePercentage || initialData.graceInvestorSharePercentage);
  ...
  useEffect(() => {
    ...
    if (typeof window !== 'undefined') {
      try {
        const appData = {
          borrowers, investors, users, supportTickets, notifications,
          salaryRepaymentPercentage, baseInterestRate, investorSharePercentage,
          graceTotalProfitPercentage, graceInvestorSharePercentage,
    ...
    isInitialLoad, borrowers, investors, users, supportTickets, notifications,
    salaryRepaymentPercentage, baseInterestRate, investorSharePercentage,
    graceTotalProfitPercentage, graceInvestorSharePercentage,
    supportEmail, supportPhone,
  ]);
  ...
  const filteredBorrowers = useMemo(() => {
    ...
    if (role === 'مستثمر') {
      return borrowers.filter(b => b.fundedBy?.some(f => f.investorId === id));
    }
  ...
  const filteredInvestors = useMemo(() => {
    ...
    if (role === 'مستثمر') {
      return investors.filter(i => i.id === id);
    }
  ...
  const updateSalaryRepaymentPercentage = (percentage: number) => { setSalaryRepaymentPercentage(percentage); toast({ title: 'تم التحديث', description: `تم تحديث نسبة السداد من الراتب إلى ${percentage}%.` }); };
  const updateBaseInterestRate = (rate: number) => { setBaseInterestRate(rate); toast({ title: 'تم التحديث', description: `تم تحديث نسبة الربح الأساسية إلى ${rate}%.` }); };
  const updateInvestorSharePercentage = (percentage: number) => { setInvestorSharePercentage(percentage); toast({ title: 'تم التحديث', description: `تم تحديث حصة المستثمر من الأرباح إلى ${percentage}%.` }); };
  const updateGraceTotalProfitPercentage = (percentage: number) => { setGraceTotalProfitPercentage(percentage); toast({ title: 'تم التحديث', description: `تم تحديث نسبة الربح الإجمالية لتمويل المهلة إلى ${percentage}%.` }); };
  const updateGraceInvestorSharePercentage = (percentage: number) => { setGraceInvestorSharePercentage(percentage); toast({ title: 'تم التحديث', description: `تم تحديث حصة المستثمر من أرباح المهلة إلى ${percentage}%.` }); };
  ...
  const registerNewOfficeManager = useCallback(async (credentials: SignUpCredentials): Promise<{ success: boolean; message: string }> => {
    ...
    systemAdmins.forEach(admin => {
        addNotification({ recipientId: admin.id, title: 'تسجيل مدير مكتب جديد', description: `المستخدم "${credentials.name}" سجل كمدير مكتب وينتظر التفعيل.` });
    });
  ...
  const updateBorrowerPaymentStatus = useCallback((borrowerId: string, paymentStatus?: BorrowerPaymentStatus) => {
      ...
      setBorrowers(prevBorrowers => {
        ...
        if (paymentStatus === 'تم السداد' && borrowerToUpdate.status !== 'مسدد بالكامل') {
          ...
          if (updatedBorrower.fundedBy && updatedBorrower.amount > 0) {
            ...
            const graceTotalProfit = updatedBorrower.amount * (graceTotalProfitPercentage / 100);
            ...
            setInvestors(prevInvestors => prevInvestors.map(inv => {
              if (investorUpdates.has(inv.id)) {
                ...
                if (update.principal > 0) newTransactions.push({ id: `t-principal-ret-${Date.now()}-${inv.id}`, date: new Date().toISOString(), type: 'إيداع رأس المال', amount: update.principal, description: `استعادة رأس مال من قرض "${updatedBorrower.name}"` });
                if (update.profit > 0) newTransactions.push({ id: `t-profit-ret-${Date.now()}-${inv.id}`, date: new Date().toISOString(), type: 'إيداع أرباح', amount: update.profit, description: `أرباح من قرض "${updatedBorrower.name}"` });
                
                return { ...inv, amount: inv.amount + update.amountToAdd, fundedLoanIds: inv.fundedLoanIds.filter(id => id !== borrowerId), transactionHistory: [...inv.transactionHistory, ...newTransactions] };
              }
              return inv;
            }));
          }
          toastMessage = { title: 'تم سداد القرض بالكامل', description: 'تمت إعادة الأموال والأرباح للمستثمرين بنجاح.' };
          return prevBorrowers.map(b => b.id === borrowerId ? updatedBorrower : b);
          ...
          if (updatedBorrower.fundedBy) {
            ...
            setInvestors(prevInvestors => prevInvestors.map(inv => {
              if (investorUpdates.has(inv.id)) {
                const update = investorUpdates.get(inv.id)!;
                return { ...inv, defaultedFunds: (inv.defaultedFunds || 0) + update.amountToDefault, fundedLoanIds: inv.fundedLoanIds.filter(id => id !== borrowerId) };
              }
              return inv;
            }));
          }
          ...
          return prevBorrowers.map(b => b.id === borrowerId ? { ...b, paymentStatus: paymentStatus } : b);
        }
      });
    ...
    }, [addNotification, toast, graceTotalProfitPercentage, investorSharePercentage, graceInvestorSharePercentage]);
      ...
      return prevInvestors.map(inv => {
          if (investorIds.includes(inv.id) && remainingAmountToFund > 0) {
              ...
              if (contribution > 0) {
                  ...
                  return { ...inv, amount: inv.amount - contribution, fundedLoanIds: [...inv.fundedLoanIds, newId] };
              }
          }
          return inv;
      });
  ...
  }, [currentUser, users, investors, addNotification, toast]);

  const addEmployee = useCallback(async (payload: NewUserPayload): Promise<{ success: boolean; message: string }> => {
    if (!currentUser || currentUser.role !== 'مدير المكتب') return { success: false, message: 'ليس لديك الصلاحية لإضافة موظف.' };
    
    const myEmployees = users.filter(u => u.managedBy === currentUser.id && u.role === 'موظف');
    if (myEmployees.length >= (currentUser.employeeLimit ?? 0)) return { success: false, message: 'لقد وصلت للحد الأقصى لعدد الموظفين.' };
    
    if (users.some(u => u.email === payload.email || u.phone === payload.phone)) return { success: false, message: 'البريد الإلكتروني أو رقم الجوال مستخدم بالفعل.' };
    
    if (!payload.password || payload.password.length < 6) return { success: false, message: 'يجب أن تتكون كلمة المرور من 6 أحرف على الأقل.' };
    ...
    const newUser: User = {
        ...
        role: 'موظف', status: 'نشط', managedBy: currentUser.id, photoURL: 'https://placehold.co/40x40.png', registrationDate: new Date().toISOString(),
    };
  ...
  const addAssistant = useCallback(async (payload: NewUserPayload): Promise<{ success: boolean; message: string }> => {
    if (!currentUser || currentUser.role !== 'مدير المكتب') return { success: false, message: 'ليس لديك الصلاحية لإضافة مساعد.' };
    
    const myAssistants = users.filter(u => u.managedBy === currentUser.id && u.role === 'مساعد مدير المكتب');
    if (myAssistants.length >= (currentUser.assistantLimit ?? 0)) return { success: false, message: 'لقد وصلت للحد الأقصى لعدد المساعدين.' };
    ...
    const newUser: User = {
        id: newId, name: payload.name, email: payload.email, phone: payload.phone, password: payload.password,
        role: 'مساعد مدير المكتب', status: 'نشط', managedBy: currentUser.id, photoURL: 'https://placehold.co/40x40.png', registrationDate: new Date().toISOString(),
  ...
  const updateUserCredentials = useCallback(async (userId: string, updates: { email?: string; password?: string }) => {
    if (updates.email) {
        const emailInUse = users.some(u => u.email === updates.email && u.id !== userId);
        if (emailInUse) {
  ...
  const updateUserStatus = useCallback((userId: string, status: User['status']) => {
      ...
      if (status === 'نشط' && userToUpdate.status === 'معلق' && userToUpdate.role === 'مدير المكتب') {
        addNotification({ recipientId: userId, title: 'تم تفعيل حسابك!', description: `مرحباً ${userToUpdate.name}، تم تفعيل حسابك كمدير مكتب. يمكنك الآن تسجيل الدخول.` });
      }

      toastMessage = (userToUpdate.role === 'مدير المكتب' && isSuspending) ? 'تم تعليق حساب المدير والحسابات المرتبطة به.' : 'تم تفعيل حساب المدير والحسابات المرتبطة به.';
  ...
  const updateUserRole = useCallback((userId: string, role: UserRole) => {
    setUsers(prev => prev.map(u => {
        if (u.id === userId) {
          ...
          if (role === 'مستثمر' || role === 'مدير النظام') { delete updatedUser.managedBy; delete updatedUser.permissions; }
          if (role !== 'مدير المكتب') { delete updatedUser.investorLimit; delete updatedUser.employeeLimit; delete updatedUser.assistantLimit; delete updatedUser.allowEmployeeSubmissions; delete updatedUser.hideEmployeeInvestorFunds; }
          if (role !== 'مساعد مدير المكتب') { if(updatedUser.permissions) delete updatedUser.permissions; } else { updatedUser.permissions = {}; }
          return updatedUser;
        }
        return u;
      })
  ...
  const updateAssistantPermission = useCallback((assistantId, key, value) => {
    setUsers(prev => prev.map(u => u.id === assistantId ? { ...u, permissions: { ...(u.permissions || {}), [key]: value } } : u));
  ...
  const deleteUser = useCallback((userId: string) => {
    ...
    if (userToDelete.role === 'مدير المكتب') {
        users.forEach(u => { if (u.managedBy === userId) idsToDelete.add(u.id); });
        managerInvestorIds = new Set(investors.filter(i => i.submittedBy === userId).map(i => i.id));
    ...
    const toastMessage = (userToDelete.role === 'مدير المكتب' && numDeleted > 1) 
  ...
  const value = useMemo(() => ({
    ...
    currentUser, filteredBorrowers, filteredInvestors, filteredUsers, supportTickets, notifications,
    salaryRepaymentPercentage, baseInterestRate, investorSharePercentage, graceTotalProfitPercentage, graceInvestorSharePercentage,
    supportEmail, supportPhone, updateSupportInfo, updateBaseInterestRate, updateInvestorSharePercentage, updateSalaryRepaymentPercentage,
    updateGraceTotalProfitPercentage, updateGraceInvestorSharePercentage, addSupportTicket, registerNewOfficeManager, addBorrower,
    updateBorrower, addInvestor, addEmployee, addAssistant, updateInvestor, withdrawFromInvestor, approveBorrower, approveInvestor,
    rejectBorrower, rejectInvestor, updateUserIdentity, updateUserCredentials, updateUserStatus, updateUserRole, updateUserLimits,
    updateManagerSettings, updateAssistantPermission, requestCapitalIncrease, deleteUser, clearUserNotifications,
    markUserNotificationsAsRead, updateBorrowerPaymentStatus
  ]);
