-- Definitive RLS Policies & Helper Function Script.
-- This is the single source of truth for database security.
-- It creates the helper function and all required security policies,
-- fixing all previous issues including SQL syntax errors.
...
-- This is comprehensive to avoid errors on re-run.
...
-- Policy 4 (CORRECT SYNTAX): Admins can UPDATE profiles.
CREATE POLICY "Admins can update profiles"
  ON public.profiles FOR UPDATE
  USING (public.get_user_role(auth.uid()) = 'مدير النظام');
  
-- Policy 5 (CORRECT SYNTAX): Admins can DELETE profiles.
